...
```yaml
표면:
  Chronicle = 그리스 신화 결의 덱빌딩 로그라이크 오토배틀러 (Unity 6.3 LTS)
...
의지 (자원):
  ...
  1라운드 1, 매 라운드 +1
  상한 7 (7라운드부터 계속 7)
  의지 비용 박힘: 액티브 키워드 / 영웅 능력 / 주문 (범위 0~7)

기본 행동 = 자동 (이동 + 공격)
  근접: 인접 1칸 공격
  원거리: 사거리 키워드 박힘 (최소 2, 최대 4)

진형:
  체스판 결 (기본 4x4, 최대 6x6)
  매 라운드 선언 단계 자유 배치 (자기 영역만)
  4방향 인접 (상하좌우)
  챕터 진행 따라 크기·모양·특수 칸 변동 (작가 결)
...
```yaml
키워드 풀 3종:
  인물 키워드     일반 인물 + 대적자 둘 다 박힘
  대적자 전용     대적자만 박힘. 인물엔 X.
  시작 키워드     회차 시작 시 1개 결단. 덱 방향 결.
                  결의 결: 시너지 / 운용 / 정체성

작동 결 3종:
  패시브        항상 작동, 의지 비용 X
  액티브        의지 비용 박힘, 선언 단계 결단
  인도 트리거   가호 발동 시 작동
```
...
```yaml
시간대 3종: 낮 / 경계 / 밤
인도 3종:   태양의 인도 / 별의 인도 / 달의 인도

라운드 시작 굴림:
  1단계: 가호 박힘 or 꽝 (50:50)
  2단계: 박히면 인도 무작위 (해당 시간대 인도 1종)
...
부대의 한 자리. 죽으면 게임 오버.
영웅 능력 = 의지 1, 매 라운드 결단 발동 가능

카테고리 5종 + 사제 분기:
  전사    (근접)
  수호자  (근접, 도발·전위)
  사냥꾼  (원거리)
  사제    (원거리, 신앙 결로 분기 — 낮/밤/경계)
  도적    (근접)
  용병    (원거리)
```
...
박힘:
  ...
  ✓ 직군 매핑 확정                                    [2026-05-12]
  ✓ 전투 시스템 큰 정정 (주사위 폐기, 의지·진형·인도 결 박힘)  [2026-05-13]
  ✓ 카드 결 큰 정정 (주문·장비 박힘, 키워드 3풀 결)            [2026-05-13]
  ✓ 보상 결 박힘 (카드 종류별 분리)                            [2026-05-13]
  ✓ 대적자 카테고리 + 사제 분기 박힘                           [2026-05-13]
...
다음 마디 후보:
  - specs/combat.md 본문 정정 (전투 시스템 큰 정정 반영)
  - specs/cards.md 본문 정정 (카드 결·키워드 결 반영)
  - specs/environment.md 본문 정정 (시간대 + 인도 결)
...
시스템:
  ...
  - 인도 효과 본문 (3 인도 × 인물별)
  - 액티브 키워드 풀의 의지 비용 결 (0~7 범위 안)
  - 주문 카드 풀 (소모성, 의지 비용)
  - 장비 카드 풀 (인물 부착, 효과군)
  - 도적·용병 키워드 풀
  - 챕터별 맵 변동 결 (크기·모양·특수 칸)
  - 특수 칸 결
  - 비대칭 맵 결
  - 늘어짐 방지 결 (7라운드 이후)

콘텐츠:
  - 인물 카드 결 정정 (위계 노출, 행동 타입, 키워드 패시브/액티브/인도)
