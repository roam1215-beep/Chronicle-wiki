# Chronicle

> 그리스 신화 결의 턴제 전술 카드 로그라이크.
> 사서 프레임 + 카타스테리스모스 + 용기·지혜·정의 세 편.

## 두 Claude 라우터

### 구현 결 (Claude Code) — 어디부터 읽나

```yaml
1. specs/cards.md       — 카드 데이터 스키마 + 예시
2. specs/combat.md      — 턴제 전투 (격돌·사격·소환·이동·적 행동)
3. specs/structure.md   — 5층 구조 + 인격 + 위계
4. specs/environment.md — 시간대 + 우호 NPC
5. content/cards/order_01_theodora/ — 실제 카드 데이터 (오더 1 팩)
```

### 기획 결 (claude.ai) — 어디부터 읽나

```yaml
1. STATE.md             — 단일 진입점
2. specs/ 전체          — 시스템 결
3. content/             — 현재 콘텐츠 결
4. design/              — 작품·서사·UI 결 (필요 시)
```

## 폴더 결

```
specs/        결정론적 명세 (스키마·알고리즘·수치)
content/      게임 데이터 (실제 카드·인물)
design/       작품 결·서사·UI
00_운영/      운영 결
STATE.md      부팅 진입점
CLAUDE.md     두 Claude 라우터
```

## 핵심 결

```yaml
구조:
  Order > Chapter > Stage > Phase > Battle (Turn)
  Order = 270 기록 카드 = 3 인격 팩
  Phase = 선택-거부-감내 3장
  Battle = 턴제 체스 (번갈아, 말 직접 지휘 + 의지로 카드)
  맵 = 8x8 (체스 결, 가로 a~h / 세로 1~8)

스탯 (3패러미터):
  공격력 + 방어력 + 생명력
  수치 결 = 첫 구현 시도 후 시뮬 결로

2축 (작품 결, faction 폐기):
  종족 (인간/무리) × 태생 (낮=이오니아 / 여명=친이오니아 / 황혼=친도리아 / 밤=도리아)
  진영은 태생으로 읽음 (faction 필드 X). 무리도 4분기 어디든 (race와 독립)

직업 (대적자만, 7종, 하스스톤 결):
  예언자·사냥꾼·음유시인·방랑자·전사·군주·사제

타입 (체스 문법 차용 — 정의 = specs/types.md):
  보병=앞1~2(후퇴X) / 기수=직교1~3 / 전령=대각1~3 / 척후=세로 한 겹 통과 / 대적자=킹(8방1)
  이동 모양 = 체스 / 전투 = 격돌(근접 동시 교환)·사격(키워드, 사냥꾼 직업색)

등급 (5종):
  보통·희귀·영웅·전설·서사
  대적자 = 서사 강제, 일반 최고 = 전설

편 (구 '인격'):
  용기 · 지혜 · 정의 = 편 (가능세계 3 = 직업 3). 위계: 전기(인물) ⊃ 오더 ⊃ 편 — 같은 편이 오더 관통(편끼리 안 섞임)
  테오도라: 용기=전사 / 지혜=사냥꾼 / 정의=사제. 기질 = "물러설 줄 모르는 자"
  회차 = 한 편. 편은 회차 단위 상태 (카드 필드 X — 직업·특기 차이로 분기)
```

## 운영

```yaml
- 본문 = 명세·데이터·결만 (변경 이력 X)
- 한 개념 = 한 정본 (SSOT)
- 결정론적 결만 specs/, 비결정론은 design/
- 청산 기조 유지 (CLAUDE.md / 운영규칙 v6.0)
```
