...
직업 매핑 결 (하스스톤 결 결 + 덱 주축):
  ...
  사제     priest     하스스톤 사제
                       주축: 축복·번  부축: 컨트롤
                       특기: 〈회복〉 — signatures.md (8방 2칸 1명 생명력 +1 회복)
                            회복을 페이오프·전환으로 활용 (멜레네 전환·스키아 누적 등)
                       축복(필드) 결 + 번(슈팅) 결 (양면 — 회복 엔진 공유, 살리거나 피해로 전환)
...
사제 priest    — 회복 엔진(특기 회복), 회복 활용처로 두 갈래. 키워드 부여·회복·피해
  -> 축복 사제(회복으로 필드 유지·페이오프) / 번 사제(회복→피해 전환·슈팅 — 멜레네·스키아)
