...
명칭은 유지, 이동 = 체스 기물 결 (5/31 확정). 이동·전투·시야 한 묶음.
  ...
  보병   soldier   — 폰 결
    이동:  자기 줄에서 앞으로 1~2칸 (방향성, 막힘 — 통과 X)
    전투:  격돌 — 바로 앞 칸만 (옆·대각·뒤 공격 X → 옆구리 약점)
    관통:  적 마지막 줄 도달 시 적 대적자에 공격력만큼 피해 후 자동 소멸
    시야:  1
...
본질:
  ...
  - 보병 = 앞만 침 → 옆구리 약점 → 배치 비가역·신중 (진형의 핵)
...
```yaml
# 데모에서 안 쓴 키워드. 턴제 재매핑 후 재정의 (강림은 데모 키워드로 이동됨).
트리거: 등장 / 치명타 / 인내 / 동조 / 퇴장
효과:   기습 / 소환 / [피해 효과]
상태:   수호 / 은신
```
