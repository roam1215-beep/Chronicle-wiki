...
전용/공용 가르는 기준 = 효과의 직업색 (타입과 독립 축 — types.md "## 카드 예산"):
  - 전용 (belongs_to: <직업>): 직업 메커니즘 보유
      전사 진형·방어도·오라·대지권능 / 사냥꾼 저격 / 사제 회복 …
  - 공용 (belongs_to: neutral): 색 없는 범용 — 바닐라 머릿수·단순 드로우·기본 유틸
  - 전령(전설)은 거의 전용 = 직업의 얼굴 (공용 전설 X가 기본)

덱 구조: 전용 20 + 공용 10 = 30
  - 전용 = 직업 색 기동(신관·전차·전령에 무게) + 발동 전부(기도·장비·권능) 전용
  - 공용 = 머릿수·기본 기동(보병에 무게), 인물만 (neutral_card: player|enemy)
  # 공용에 보병이 쏠리고 전용에 에이스 타입이 쏠리는 건 정상 — 위 기준의 자연한 결과.
```
