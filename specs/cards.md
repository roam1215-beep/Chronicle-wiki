...
직업 매핑 결 (하스스톤 결 결 + 덱 주축):
  ...
  사냥꾼   hunter     하스스톤 사냥꾼
                       주축: 사격·어그로  부축: 기동
                       특기: 사용 턴 임시 공격력 +1 + 사격(키워드, 8방향 1칸·시야 안 적)
...
전사 warrior   — 말 하나하나가 강한 필드 싸움.    키워드 부여·보호·돌격
  -> 장비 전사 / 부여 전사
사냥꾼 hunter   — 사격·적 대적자 직격 어그로.      키워드 신속·돌격·쇄도·사격
  -> 돌격 사냥꾼 / 신속 사냥꾼
...
사격:  8방향 1칸 일방 공격 부여 (반격 X·시야 안 적·쏘면 노출[밤 제외]) — 사냥꾼 직업색. 점수 = 시뮬 (상세 combat.md·types.md)
...
```yaml
의식:  사제 특기 — 발동 시 의식 스택 충전 (기존 hp+1 특기를 대체).
