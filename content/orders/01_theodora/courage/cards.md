...
> 90장 = 9 스테이지 × 10장. 현재 박힘: 챕터 1 스테이지 1 (라키아의 들개)
...
- id: c1_s1_p2_card_2
  ...
  effect: "[펜딩] 우호 NPC 영입 자리? 토벌대 = 환경 모디파이어 자리?"
...
```yaml
# 스테이지 명만 박힘 (도망치는 자들 / 다가오는 위협 / 헤매임의 끝)
# 페이즈 + 카드: [펜딩]
```
...
```yaml
# 스테이지 명만 박힘 (귀환 / 기만과 체념 사이 / 변하지 않는 것들)
# 페이즈 + 카드: [펜딩]
```
...
- TODO(시스템): 우호 NPC 영입 결 (페이즈 어디서?)
- TODO(시스템): 토벌대 환경 모디파이어 결
```
